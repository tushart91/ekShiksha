        ...
        if(error.equals(",")) {
            ...
            if(!userHandler.isUserIdExist(userId)) {
                if(!userHandler.isEmailExist(email)) {
                    if(userHandler.createUser(userId, password, displayName, "user", qualification, email, Long.parseLong(mobile), profile, Utility.captalizeFirstChar(firstName), Utility.captalizeFirstChar(lastName))) {
                        error = ",success,";
                        result = true;
