...
<%
    try {
        String unicode = "";
        int status = 3; //status = 3 ,it means that content is reviewed
...
<%
        ...
        if(type!=null && type.equals("complex")) {
            if(helper.reviewComplexContent(topicId, linkId, langId, contentId, unicode, status)) {
            %>
            <br>
            <h3 align="center">Review successful</h3>
            <p align="center">Click here to<a href="javascript:window.close();"> close </a>the window.</p>
            <%
            } else {
            %>
            <br>
            <h3 align="center">Review is not successful</h3>
            <p align="center">Click here to<a href="javascript:window.close();"> close </a>the window.</p>
            <%
            }
