...
<%
    ...
    if(role != null && userId != null && tLangId != null && queryString != null) {
        try {
            ...
            translatorLangId = Integer.parseInt(tLangId);
            url = helper.getLink(topicId, linkId, translatorLangId);
        } catch(Exception e) {
            isError = true;
        }
    } else {
...
<div class="infodiv"><bean:message key="requesttranslation.form.infoone"/> <a href="#"><%=url.getDesc()%></a>, <bean:message key="requesttranslation.form.infotwo"/></div>
...
<div id="notaccept" class="infowrap">
    ...
    <ul style="list-style:circle;margin-top:5px;margin-left: 20px;">
        ...
        <li><bean:message key="requesttranslation.form.limiterror"/></li>
        <li><bean:message key="translatetitle.form.errortwo"/></li>
    </ul>
</div>
<div id="accept" class="infowrap">
    <span><bean:message key="requesttranslation.form.success"/></span>
    <p><bean:message key="reqeusttranslation.form.translatemessage"/> <a href="showTranslationPage.do?topicId=<%=topicId%>&linkId=<%=linkId%>"><bean:message key="translatetitle.form.translate"/></a></p>
</div>
...
<script type="text/javascript">
    ...
    function requestTranslation(topicId, linkId) {
        ...
        //ajax call
        $.get("pages/translationrequest.jsp",{topicId: topicId, linkId: linkId}, function(message) {
