...
<%
    ...
    String rLangId = (String)session.getAttribute("publisherLangId");
    ...
    if(role != null && userId != null && rLangId != null && queryString != null) {
        try {
            ...
            publisherLanguage = Integer.parseInt(rLangId);
            originalLanguage = handler.getOriginalLanguage(topicId, linkId);
            url = helper.getLink(topicId, linkId, originalLanguage);
            reviewedTitle = helper.getLinkDescription(topicId, linkId, publisherLanguage, 4);
            if(reviewedTitle == null || reviewedTitle.length() < 1) {
                reviewedTitle = helper.getLinkDescription(topicId, linkId, publisherLanguage, 3);
                if(reviewedTitle == null || reviewedTitle.length() < 1) {
                    reviewedTitle = helper.getLinkDescription(topicId, linkId, publisherLanguage, 2);
                    if(reviewedTitle == null || reviewedTitle.length() < 1) {
                        reviewedTitle = url.getDesc();
                    }
                }
                isReviewed = false;
            }
...
%>
    ...
    <form name="publishTitleRequest" action="pages/titlepublishrequest.jsp" method="post" id="publishTitleRequest" style="margin-top:10px;">
        <%
            if(!isReviewed) {
        %>
        <div  style="color:#a71c27;margin-left: 10px;margin-bottom: 13px;"><bean:message key="publishtitle.form.nottranslated"/></div>
        <%
            }
        %>
        <div class="inputrow">
        ...
        <div class="inputrow">
            <span><bean:message key="publishtitle.form.reviewedtitle"/> : <strong>*</strong></span>
            <textarea id="reviewedTitle" id="reviewedTitle"><%= reviewedTitle %></textarea>
            <div id="publishTitleError" class="errormsg"><bean:message key="translatetitle.form.blanktitle"/></div>
        </div>
        ...
        <div class="action-layer">
            <button class="actionbutton" type="submit" id="submit"><bean:message key="publishtitle.form.submit"/></button>&nbsp;
             <button class="actionbutton" type="reset" id="reset" ><bean:message key="label.button.reset"/></button>&nbsp;
             <button class="actionbutton" onclick="VirtualKeyboard.toggle('reviewedTitle','kb'); return false;" type="button"><bean:message key="label.button.keyboard"/></button>
        </div>
...
<div id="accept" class="infowrap">
    <span><bean:message key="publishtitle.form.success"/></span>
</div>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function() {
        $("#reviewedTitle").focus();
        $("#publishTitleRequest").submit( function() {
           //Get the data from all the fields
           var topicId = $('input#topicId').val();
           var linkId = $('input#linkId').val();
           var translatedTitle = $('textarea#reviewedTitle').val();
           //clearing previous errors
           $("#publishTitleError").hide();
           $("#reviewedTitle").removeClass("error");
           //Simple validation to make sure user entered something
            //If error found, show error message
           if(translatedTitle == "") {
               $("#publishTitleError").show();
               $("#reviewedTitle").addClass("error");
               $("#reviewedTitle").focus();
               return false;
           }else {
               //organize the data properly
                var data = "topicId=" + topicId + "&linkId=" + linkId + "&publishedTitle="
                + translatedTitle;
               //start the ajax
               $.ajax({
                  //this is the jsp file that processes the data
                  url: "pages/titlepublishrequest.jsp",
                  //POST method is used
                  type: "POST",
                  //pass the data
                  data : data,
